...
import { listPagedService } from "../services/clientesService";
import { createClienteSchema, updateClienteSchema } from "../validators/clientesValidator";
...
export async function search(req: Request, res: Response) {
    ...
    const telefone = String((req.query as any)?.telefone ?? "").trim();
    const q = String((req.query as any)?.q ?? "").trim();

    const out = await svc.searchClientes({ nome, email, telefone, q, limit, offset });
