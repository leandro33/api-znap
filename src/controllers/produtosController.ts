...
import { listPagedService } from "../services/produtosService";
import { createProdutoSchema, updateProdutoSchema } from "../validators/produtosValidator";
...
export async function search(req: Request, res: Response) {
    ...
    const categorias_id = Number((req.query as any)?.categorias_id ?? null);
    const preco = Number((req.query as any)?.preco ?? null);
    const q = String((req.query as any)?.q ?? "").trim();

    const out = await svc.searchProdutos({ nome, descricao, categorias_id, preco, q, limit, offset });
...
export const remove = async (req: Request, res: Response) => {
    ...
    if (Boolean(removed) === false) throw new NotFoundError("cliente não encontrado");
    res.send(204);
};
